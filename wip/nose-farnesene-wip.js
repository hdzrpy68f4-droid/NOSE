(() => {
  ...
  function step(name,fn){ try{ fn(); }catch(err){ if(window.console&&console.warn) console.warn('[NOSE] '+name+' skipped:',err&&err.message); if(window.__noseReport) window.__noseReport('init:'+name,{message:err&&err.message,source:err&&err.stack}); } }
    ...
    const TERPENES = {
      ...
      farnesene:{name:'β-Farnesene',family:'herbal'}
    ...
    const TERP_ALIAS={
      'alpha-pinene','a-pinene','α-pinene','pinene-a',
      'beta-pinene':'pinene_b','b-pinene':'pinene_b','β-pinene':'pinene_b','pinene-b':'pinene_b',
      'beta-caryophyllene':'b-caryophyllene': 'β-caryophyllene':
      ...
      'trans-b-farnesene':'farnesene','trans-beta-farnesene':'farnesene',
'alpha-farnesene':'farnesene','beta-farnesene':'farnesene','β-farnesene':'farnesene',
// Oxides and bare 'pinene' are deliberately absent: caryophyllene oxide and
// linalool oxide are distinct compounds, and 'pinene' is ambiguous between
// pinene_a and pinene_b. Unknown keys are dropped by sanitizeTerps.
