...
fetcher behave exactly as they did before. Wiring it is a separate job.

**The database is Supabase Postgres**, reached with the `pg` driver over a
connection string in `NOSE_DB_URL`. Netlify Database was considered and
declined: it is available on credit-based plans only. That decision lives here
so the next session does not re-litigate it — if the plan ever changes, the
migration is plain DDL and moves across unchanged.

**Use the transaction pooler (port 6543), not the direct connection.** Netlify
Functions reach out over IPv4; Supabase's direct connection is IPv6 unless the
project buys the IPv4 add-on. Transaction mode also forbids prepared
statements, which is why no query in `store.js` is given a `name` — adding one
will fail in production and pass everywhere else.

### What exists

```
db/migrations/0001_nose_coa_store.sql   documents, parses, terpene_values
netlify/functions/lib/store.js          saveDocument, saveParse, end
test/store-test.js                      skips unless NOSE_TEST_DB_URL is set
netlify/functions/lib/parser-version.js generated by build.sh, committed
```

- `saveDocument({sha256, sourceUrl, bytes, text, extractorVersion})` upserts on
`sha256` and returns the document id. The same bytes saved twice yield one row
and the same id. One statement: a CTE insert with `ON CONFLICT DO NOTHING`,
`UNION ALL` the existing row.
- `saveParse(documentId, parserVersion, output)` writes the `parses` row and
one `terpene_values` row per key in `output.terps`, **zeros included** — a
below-LOQ 0 is a reading, not an absence — both inside one transaction. A parse
row whose terpene rows failed to land would be a fingerprint that silently lost
compounds.

**Every numeric column is `numeric`, never `float8`.** Not for display: float8
prints `4.124` back correctly. For arithmetic. `0.1 + 0.2 = 0.3` is false in
float8 and true in numeric, and this parser's whole discipline is reconciling
sums against the total a lab printed.

**`harvest_date` and `report_date` are `text`.** Labs print several formats and
some print none. Coercing to `date` would make the store assert a reading the
parser never made.

**`report_date` and `client` are NULL on every row.** The parser's output
contract (§7) does not emit them. The columns are there for when it does;
`client` is the licensee named on the COA, which is a business.

### The privacy line

**No column in these tables identifies a person** — no user id, no IP, no
device, no palate, no session. `output` is stored wholesale as jsonb, which is
safe only because §7 contains no such field today. `assertNoPersonalFields()`
in `store.js` is the tripwire if that ever changes: it throws rather than
writing. It deliberately does not block `terps` or anything aroma-related.

If a future feature needs to associate a parse with a person, that association
does not belong in these tables.

### Tests

`test/store-test.js` reads **`NOSE_TEST_DB_URL`, never `NOSE_DB_URL`** — it
writes rows, and pointing it at the live database would seed production with
fixtures. It refuses outright if the two are set to the same string, and skips
with a message (exit 0) when the test URL is unset, so the gate sequence still
runs on a fresh clone. It verifies through its own connection rather than
...
`build.sh` writes the git short SHA into
`netlify/functions/lib/parser-version.js` on every build, and the file is
committed — same pattern as the fingerprinted bundles.

It is a file rather than an environment variable because **Netlify does not
pass build environment variables through to functions at runtime.** Only `URL`,
`SITE_NAME`, `SITE_ID` and variables explicitly given Functions scope arrive
there; `COMMIT_REF` does not. An exported shell variable in `build.sh` would be
gone by the time a function runs, and `parser_version` would have been silently
`undefined` on every stored row. At wiring time, read it as:

```
const parserVersion = process.env.PARSER_VERSION || require('./lib/parser-version.js');
```

### While doing this, two unrelated faults were found and fixed

- **`js/nose.33a9ba4f.js` was committed under a stale name** — its bytes hash to
`a69ec26a`. The filename had stopped matching its content, so the cache-busting
name was no longer busting anything and a browser holding the old bundle would
keep serving it. Any `build.sh` run renames it; the rename is now committed.
The other three bundles all matched their names.
- **`build.sh` lost its executable bit** (100755 → 100644) when it was replaced
through the editor. `netlify.toml` runs `bash build.sh`, so the deploy was
unaffected, but `./build.sh` would have failed. Restored.
