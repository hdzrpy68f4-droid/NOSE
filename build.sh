...
echo "==> recording parser version"

# The parser version stamped onto every stored parse. Netlify does NOT pass
# build environment variables through to functions at runtime -- only URL,
# SITE_NAME, SITE_ID and variables explicitly given Functions scope -- so an
# exported shell variable here would be gone by the time a function runs.
# Writing it into a module that ships with the function bundle is what works.
PARSER_VERSION=$(git rev-parse --short HEAD 2>/dev/null || echo unknown)
export PARSER_VERSION
printf "'use strict';\n/* generated by build.sh - do not edit */\nmodule.exports = '%s';\n" \
  "$PARSER_VERSION" > netlify/functions/lib/parser-version.js
