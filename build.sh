...
# What every stored parse is stamped with, and whether this deploy may store
# anything at all. Netlify does NOT pass build environment variables through to
# functions at runtime -- only URL, SITE_NAME, SITE_ID and variables explicitly
# given Functions scope -- so an exported shell variable here would be gone by
# the time a function runs. lib/version.js writes build-info.json instead,
# which esbuild bundles into the function:
#   parserVersion     git short SHA
#   extractorVersion  short hash of extract-text.js + the installed unpdf
#   deployContext     $CONTEXT - only "production" stores anything, because
#                     deploy previews share the site's Blobs store and would
#                     write into the real archive
# Generated and gitignored; unset CONTEXT (a Codespace) records "dev".
node netlify/functions/lib/version.js --write \
  || { echo "FAIL: could not write netlify/functions/lib/build-info.json"; exit 1; }
...
[ "$(grep -rho 'style="' --include='*.html' --exclude-dir=node_modules --exclude-dir=netlify --exclude-dir=test . | wc -l)" -eq 0 ] \
